...
def calculate_relative_change(df, comp, comparison_reference, on_vars):
    ...
    df_comp = df_comp.rename(columns={
        "scenario_id": "scen_comp", "end_value": "value_comp"})[
        ["scen_comp", "value_comp"] + on_vars]
...
def sample_df(df, scenario, pathogen, k=1000):
    ...
    # summary of changes: (1) line 217 (2) remove redundant list() and int() (3) use np as much as possible
    if len(scenario) > 0:
        p1 = time.time()
        df_scen = df[df["scenario_id"].isin(scenario)].copy()
        df_scen["sample_id"] = (
                    df_scen["model_name"].astype(str) + "_" + df_scen["type_id"].astype(str) + "_" +
        ...
        for model in df_scen["model_name"].drop_duplicates():
            df_model = df_scen[
                df_scen["model_name"] == model]  # I think this can be combined with the below
            len_df = len(df_model[(df_model["scenario_id"] == scenario[0]) &
                                  (df_model["horizon"] == min(df_model["horizon"]))])
            list_sample.append(df_model[
                                   "sample_id"].unique())  # I think this can be moved out of the for-loop (isn't it all sample ids?)
            weight_list = [1 / len_df] * len(df_model[
                                                 "sample_id"].unique())  # why is this df_model instead of the filtered thing corresponding to len_df?
        ...
        weight_sample_fin = flatten_list(weight_sample) / np.array(
            len(df_scen["model_name"].drop_duplicates()))
        weight_sample_fin = weight_sample_fin / len(scenario)

        all_sample_sel = np.random.choice(list_sample, p=weight_sample_fin, size=k, replace=True)
        all_sample_sel = np.random.permutation(
            all_sample_sel)  # not sure if necessary but doesn't add much time

        df_sample_sel = df_scen.reset_index().set_index("sample_id").loc[
            all_sample_sel]  # note: very large dataset
        all_sample = df_sample_sel.reset_index().drop(columns="index")

        p2 = time.time()
        # sample_idxs = all_sample["sample_id_n"] = list(np.repeat(list(range(k)), len(df_scen['horizon'].drop_duplicates())))
        sample_idxs = all_sample["sample_id_n"] = np.repeat(np.arange(k), len(
            df_scen['horizon'].drop_duplicates()))
        all_sample["sample_id_n"] = sample_idxs

        p3 = time.time()
        all_sample = all_sample[["value", "target_end_date", "sample_id_n"]]
        all_sample = all_sample.rename(columns={"value": "value" + "_" + pathogen})
        p4 = time.time()
        # print(f'sample_df: Sampling trajectories = {p2-p1}, creating sample_id_n col = {p3-p2}, cleaning up = {p4-p3}')
    else:
        all_sample = pd.DataFrame(
            columns=["value" + "_" + pathogen, "target_end_date", "sample_id_n"])
    return all_sample
...
def prep_multipat_plot_comb(pathogen_information, calc_mean=False):
    ...
    """
    # Changed all quantile calcs to use numpy instead of pandas
    all_sample = pd.DataFrame()
    ...
    p1 = time.time()
    all_quantile = all_sample.groupby(["target_end_date"]).agg(f)  # approx. 0.6
