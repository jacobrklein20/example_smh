...
app.layout = html.Div([
    html.Div(
        [
            html.Div([
                dcc.Link("Scenario Definition",
                         href=dash.page_registry["pages.scenario_definition"]["path"],
                         className="menu_pages"),
                dcc.Link("Model Metadata", href=dash.page_registry["pages.model_metadata"]["path"],
                         className="menu_pages"),
                dcc.Link("Plots", href=dash.page_registry["pages.plots"]["path"],
                         className="menu_pages"),
            ]),
            html.Br()
        ]
...
def query_proj_data(location, target, type_name, round_number,
                    path="./visualization/data-visualization/models/",
                    filename="part-0"):
    if location is None:
        file_name = path + "round" + str(
            round_number) + "/" + target + "/" + type_name + "/" + filename
    else:
        file_name = (path + "round" + str(
            round_number) + "/" + target + "/" + location + "/" + type_name + "/" +
                     filename)
    if type_name == "sample":
...
def scenario_plot_prep(scenario, location, target, ui, age_group, ens_check, round_tab):
    ...
    if (df is None) or (len(df) == 0):
        ...
        for scen in df["scenario_id"].unique():
            title = ("<br>Scenario " + re.findall("^\\D", constant_dict["scenario_id"][str(scen)])[
                0] + ": " +
                     re.sub(", | and ", ",<br>",
                            scen_info[constant_dict["scenario_id"][str(scen)]]))
            sub_scen.append(title)
    title = (
                "<b>Projected " + y_title.title() + " by Epidemiological Week and by Scenario for " + round_tab + " (" +
                age_group + ")")
    # zoom
    ...
    prep_scenario = {"df": df, "df_gs_data": df_gs_data, "intervals": intervals,
                     "x_title": "Epiweek",
                     "y_title": y_title, "hover_text": "Model Name: %{model_name}<br>",
                     "ensemble_name": viz_setting[round_tab]["ensemble"]["default"],
                     "h_lines": h_lines,
                     "ensemble_color": "rgba(0, 0, 0, 1)", "ensemble_view": True,
                     "color_dict": color_dict,
                     "title": title, "subplot_title": sub_scen, "point_value": point_value,
                     "truth_data_type": truth_data_type, "zoom_in_projection": zoom_dict,
                     "v_lines": v_lines,
...
def spaghetti_plot_prep(scenario, location, target, age_group, n_sample, med_plot, round_tab):
    ...
    if n_sample < 20:
        ...
        opacity = 0.1
    if (df is None) or (len(df) == 0):
        prep_plot = {"df": None, "subplot_titles": None, "title": "", "opacity": opacity,
                     "color_dict": color_dict,
                     "y_title": target_dict[target]}
        return prep_plot
    ...
    if len(df) > 0:
        ...
        df_sel = list()
        for scen in df["scenario_id"].unique():
            title = ("Scenario " + re.findall("^\\D", constant_dict["scenario_id"][str(scen)])[
                0] + ": " +
                     re.sub(", | and ", ",<br>",
                            scen_info[constant_dict["scenario_id"][str(scen)]]))
            sub_scen.append(title)
        ...
        df = pd.concat(df_sel)
        title = "Projected " + target_dict[
            target] + " by Epidemiological Week and by Scenario for " + round_tab
        ...
        prep_plot = {"df": df, "subplot_titles": sub_scen, "title": title, "opacity": opacity,
                     "color_dict": color_dict,
                     "y_title": target_dict[target]}
    else:
        prep_plot = {"df": None, "subplot_titles": None, "title": "", "opacity": opacity,
...
def specific_plot_prep(model, location, target_type, ui, age_group, round_tab):
    ...
    # Data
    df_all = prep_model_data(sel_target, location, model, quant_sel, age_group, round_number,
                             invert_team_name)
...
def comparison_plot_prep(location, age_group, multi_ref, round_tab):
    ...
    if "multi_panel" in viz_setting[round_tab]["scenario_comparison"]:
        comparison_reference = viz_setting[round_tab]["scenario_comparison"]["multi_panel"][
            multi_ref]
    ...
    if len(df) == 0:
        ...
        title = (
                    "Excess Percentage of reported hospitalizations compared with the counterfactual scenario (" +
...
def peak_time_prep(scenario, model, order, round_tab):
    ...
    if order == "Geographical":
        loc_list = pd.concat(
            [loc_list, (df_sort_location(["US"] + constant_dict["location_order"]))])
    ...
    for patho in patho_selected:
        ...
        if df_gs_other_data is not None:
            if "target" in df_gs_other_data.columns:
                ...
                df_gs_other_data = df_gs_other_data[
                    ["time_value", "geo_value_fullname", "fips", "value"]]
    ...
    if df_gs_data is not None:
        ...
        if "peak" in viz_setting[round_tab]["multi-pathogen_plot"]:
            ...
            for peak_name in viz_setting[round_tab]["multi-pathogen_plot"]["peak"]:
                ...
                peaks.append(
                    {"name": " + ".join(patho_selected + [pathogen_name]) + " " + peak_name,
...
def multi_pathogen_combined_prep(location, target, scenario, other_scen, round_tab, k=10000):
    ...
    for patho in other_pathogen:
        if other_scen[patho] is not None and len(other_scen[patho]) > 0:
            df_other = query_proj_data(location, target, "sample",
                                       other_pathogen_data[patho]["round_number"],
    ...
    for patho in list(pathogen_info.keys()):
        ...
        df_patho = prep_pathogen_data(df_patho, pathogen_info[patho]["scenario_int"], patho.lower(),
                                      k=k)  # just a wrapper on sample_df from smhViz_plot utils
    ...
    obs_data = multi_pathogen_obs_prep(round_tab, target, location, obs_other_patho,
                                       time_series_date)
    ...
    title = (viz_setting[round_tab]["target"][
                 target] + " - " + location + " (" + pathogen_disp_name + " (" +
    ...
    for patho in other_pathogen:
        ...
        if other_scen[patho] is not None and len(other_scen[patho]) > 0:
            ...
            other_scen_text.append(
                pathogen_set[patho]["display_name"] + ": " + "no scenario selected")
        else:
            other_scen_text.append(
                pathogen_set[patho]["display_name"] + ": " + "no projection available")
    title = (
                title + "<br><span style='font-size:14px'>" + pathogen_disp_name + ":  Scenario " + scen_text + "; " +
                "; ".join(other_scen_text) + "</span>")
    # Output
    prep_plot = {"df_all": df_all, "df_gs_data": obs_data["df_gs_data"], "title": title,
                 "peaks": obs_data["peaks"],
                 "truth_legend": pathogen_disp_name + " Observed Data",
                 "target": viz_setting[round_tab]["target"][target],
                 "truth_tot_legend": pathogen_disp_name + " + " + " + ".join(
                     title_other_pathogen).title() +
                                     " Observed Data"}
...
def draw_scenario_plot(scenario, location, target, ui, age_group, ens_check, round_tab, w_delay=6):
    if len(scenario) < 1:
        ...
        prep_scen = scenario_plot_prep(scenario, location, target, ui, age_group, ens_check,
                                       round_tab)
        target_dict = viz_setting[round_tab]["target"]
        # Plot
        if (prep_scen["df"] is None) or (len(prep_scen["df"]) == 0):
            fig = fig_error_message(
                "No projection to display for the target: " + target_dict[target] + ", location: " +
            ...
            fig = make_scatter_plot(df, prep_scen["df_gs_data"], intervals=prep_scen["intervals"],
                                    ...
                                    ensemble_name=prep_scen["ensemble_name"],
                                    title=prep_scen["title"],
                                    ensemble_color=prep_scen["ensemble_color"],
                                    h_lines=prep_scen["h_lines"],
                                    ensemble_view=prep_scen["ensemble_view"],
                                    color_dict=prep_scen["color_dict"],
                                    subplot_title=prep_scen["subplot_title"],
                                    point_value=prep_scen["point_value"],
                                    truth_data_type=prep_scen["truth_data_type"], share_x="all",
                                    share_y="all",
                                    zoom_in_projection=prep_scen["zoom_in_projection"],
                                    v_lines=prep_scen["v_lines"],
                                    truth_legend_name=prep_scen["truth_legend_name"],
                                    w_delay=w_delay)
    ...
    fig.add_annotation(
        x=0, y=1, xref="paper", yref="paper", text="&#9432;", font=dict(size=32),
        arrowcolor="white",
        arrowhead=False,
...
def draw_spaghetti_plot(scenario, location, target, age_group, n_sample, med_plot, round_tab):
    prep_plot = spaghetti_plot_prep(scenario, location, target, age_group, n_sample, med_plot,
                                    round_tab)
    if (prep_plot["df"] is None) or (len(prep_plot["df"]) == 0):
        fig = fig_error_message(
            "No projection to display for the target: " + prep_plot["y_title"] + ", location: " +
        ...
        if med_plot is not None and med_plot == [True]:
            fig = make_spaghetti_plot(prep_plot["df"], subplot=True, subplot_col="scenario_id",
                                      color_dict=prep_plot["color_dict"],
                                      opacity=prep_plot["opacity"],
                                      add_median=True, title=prep_plot["title"], x_title="Epiweek",
                                      subplot_titles=prep_plot["subplot_titles"],
                                      y_title=prep_plot["y_title"],
                                      legend_dict=constant_dict["model_name"])
        else:
            fig = make_spaghetti_plot(prep_plot["df"], subplot=True, subplot_col="scenario_id",
                                      color_dict=prep_plot["color_dict"],
                                      opacity=prep_plot["opacity"],
                                      title=prep_plot["title"],
                                      subplot_titles=prep_plot["subplot_titles"],
...
def draw_specific_plot(model, location, target_type, ui, age_group, round_tab):
    ...
    if (prep_spec["df"] is None) or (len(prep_spec["df"]) == 0):
        fig = fig_error_message(
            "No projection to display for the Model: " + str(model) + ", Location: " +
            str(location) + "; please select other options")
    else:
        fig = make_scatter_plot(prep_spec["df"], prep_spec["df_gs_data"], subplot_var="target",
                                line_width=4,
                                legend_col="scenario_id", x_title="Epiweek", y_title="",
                                title=prep_spec["title"],
                                point_value=prep_spec["point_value"],
                                intervals=prep_spec["intervals"],
                                subplot_title=prep_spec["subplot_title"], share_x=False,
                                share_y=False,
...
def draw_comparison_plot(location, age_group, multi_ref, round_tab):
    ...
    if prep_comp["df"] is None:
        ...
        fig = make_point_comparison_plot(prep_comp["df"], prep_comp["ens_name"],
                                         prep_comp["comparison"],
    ...
    if len(scenario) < 1:
        ...
        if len(prep_plot["df"]) > 0:
            fig = make_heatmap_plot(prep_plot["df"], subplot=True, subplot_col="scenario_id",
                                    subplot_titles=prep_plot["subplot_title"],
                                    title="Hospitalization Peak Timing")
            fig.add_annotation(
                x=0, y=1, xref="paper", yref="paper", text="&#9432;", font=dict(size=32),
                arrowcolor="white",
                arrowhead=False,
            ...
            fig.add_vline(x=datetime.today().strftime('%Y-%m-%d'), line_width=2, line_color="gray",
                          line_dash="dash")
        else:
    ...
    if len(scenario) < 1 and scen_other_len < 1:
        ...
        for patho in other_scen.keys():
            if other_scen[patho] is not None and len(other_scen[patho]) >= 1:
                other_pathogen.append(
                    viz_setting[round_tab]["multi-pathogen_plot"]["pathogen"][patho][
                        "display_name"])
        ...
        fig = make_combine_multi_pathogen_plot(
            df_all, [constant_dict["pathogen_display_name"]] + other_pathogen,
            title=prep_plot["title"],
        ...
        if None not in prep_plot["peaks"]:
            for peak_dict in prep_plot["peaks"]:
                if peak_dict["value"] is not None and ~np.isnan(peak_dict["value"]):
                    fig = fig.add_hline(y=int(peak_dict["value"]), line_width=1, line_color="black",
                                        line_dash="dash",
                                        annotation=dict(font_size=10, font_color="black"),
                                        annotation_position="top left", row=1,
                                        annotation_text=peak_dict["name"] + ": " + str(
                                            int(peak_dict["value"])))
        title_pathogen = list()
        for patho in other_pathogen:
            if "round_display" in viz_setting[round_tab]["multi-pathogen_plot"]["pathogen"][
                patho.lower()].keys():
                round_name = ("Round" +
                              viz_setting[round_tab]["multi-pathogen_plot"]["pathogen"][
                                  patho.lower()]["round_display"])
            else:
                round_name = \
                viz_setting[round_tab]["multi-pathogen_plot"]["pathogen"][patho.lower()][
                    "round"].title()
            title_pathogen.append(patho + " (" + round_name + ")")
        if "notes" in viz_setting[round_tab]["multi-pathogen_plot"]:
            fig.add_annotation(x=0, y=-0.075,
                               text="".join(viz_setting[round_tab]["multi-pathogen_plot"]["notes"]),
                               xref="paper", yref="paper", arrowhead=False, arrowcolor="white",
                               xanchor="left",
                               yanchor="top")
        fig.add_annotation(
            x=0, y=1, xref="paper", yref="paper", text="&#9432;", font=dict(size=32),
            ...
            hovertext="These projections are generated by combining randomly sampled " +
                      constant_dict["pathogen_display_name"] + " " + round_tab + " "
                                                                                 "<br>trajectories with randomly sampled " + ", ".join(
                title_pathogen) + " trajectories. "
...
def render_scenario_content(round_tab):
    round_number = viz_setting[round_tab]["round_number"]
    filename = "visualization/data-visualization/scenario_info/scenario_round" + str(
        round_number) + ".md"
...
def render_abstract_content(round_tab, team_model_name):
    round_number = viz_setting[round_tab]["round_number"]
    abs_content = render_abstract(str(round_number), viz_setting[round_tab]["date_round"],
                                  team_model_name)
    return abs_content
...
def render_sidebar_content(plot_tab, round_tab):
    ...
    if 'risk_map' in viz_setting[round_tab].keys():
        if viz_setting[round_tab]["risk_map"]["cumulative"] == "False":
            cumulative = False
    return make_sidebar(round_number, plot_tab, scen_file, location_rsv, scenario_dict, target_dict,
                        def_target,
...
def render_plot_tab_content(plot_tab, round_tab):
    ...
    scen_information = scen_df[
        scen_df["round"] == "round" + str(viz_setting[round_tab]["round_number"])]
    ...
    if "multi-pathogen_plot" in viz_setting[round_tab]:
        ...
        for patho in viz_setting[round_tab]["multi-pathogen_plot"]["pathogen"]:
            ...
            patho_dic = {"scenario": patho_scen_dict, "default_sel": def_scen,
                         "round_int": patho_round,
        ...
        if plot_tab == "multipat_plot_comb":
            radio = html.Div([
                html.Span("Error bar: "),
                dcc.RadioItems(radio_error_bar_list, radio_error_bar_list[0], inline=True,
                               id="comb_err_radio")
            ], className="err_bar")
    else:
        other_pathogen = None
    plot_bar = make_plot_bar(ens_def, max_horizon, hide_ens, sc_panel, sc_multi_panel,
                             sc_sidebar_option, pathogen,
            ...
            for patho in pathogen_name:
                ...
                if os.path.isfile(other_file_name):
                    file_list.append(dcc.Tab(
                        children=[html.Div(
                            [html.Embed(src=other_file_name, className="scenario_table")])],
                        label=viz_setting[round_tab]["multi-pathogen_plot"]["pathogen"][patho][
                            "display_name"]))

            return html.Div([
                dcc.Tabs(parent_className="plot_tabs", className='plot_tabs-container',
                         children=file_list)
            ])
        else:
            return html.Div([
